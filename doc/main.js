  ...
  if (gistContainer.length > 0) {
    $.get("https://api.github.com/users/LaurensRietveld/gists", function(data) {
      var processLabel = function(origLabel) {
        var label = origLabel.replace("#YATE", "YATE");
        label = label.replace("#YASR", "YASR");
        var splitted = label.split(" ");
        if (splitted.length > 0) {
          if ((splitted[0].indexOf("YATE") == 0 || splitted[0].indexOf("YASR") == 0) && splitted[0].slice(-1) == ":") {
            //we want to change "#YATE: some gist" into "some gist". So, remove the first item
      ...
      data.forEach(function(gist) {
        if (gist.description.indexOf("#YATE") >= 0) {
